...
function EnigmaPark(capacity, actions) {
    ...
    const park = (car, cb) => {
        setTimeout(() => {
            if (parkingLot.remaining === 0) {
                cb('Mohon maaf parking sudah penuh.')
    ...
    const leave = (plateNumber, cb) => {
        setTimeout(() => {
            ...
            if(!car){
                cb(`Mobil denga Nopol ${plateNumber} tidak ada.`)
            } else {
                const slotNumber = parkingLot.cars.findIndex((car) => car.plateNumber === plateNumber)
                parkingLot.remaining += 1
                // kalo ga bisa pake splice, ga slice
                parkingLot.cars = parkingLot.cars.splice(slotNumber, 1)
...
// manggil fungsi di dalam terus meneru
